...
#define TEMP_SET1 _IOWR(IOCTL_MAGIC_NUMBER, 0, int)
...
long TEMP_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
   ...
   switch(cmd) {
      case TEMP_SET1:
         ...
         //*gpsel1 &= ~(1<<25);
         *gpsel1 |= (1<<18);   
         *gpclr1 |= (1<<16);   //digitalWrite(DHpin, LOW); // bus down, send start signal
         mdelay(18);       // delay greater than 18ms, so DHT11 start signal can be detected
         *gpset1 |= (1<<16);   //digitalWrite(DHpin, HIGH);
...
int __init TEMP_init(void){
   if(register_chrdev(TEMP1_MAJOR_NUMBER, TEMP1_DEV_NAME, &TEMP_fops) < 0)
...
void __exit TEMP_exit(void){
   unregister_chrdev(TEMP1_MAJOR_NUMBER, TEMP1_DEV_NAME);
